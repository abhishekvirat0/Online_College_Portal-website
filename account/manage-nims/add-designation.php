...
<head>
  ...
  <script type="text/javascript">
      ...
      {
        setTimeout(function(){
          $('#light_box').load('add-designation-update-modal.php?id='+id); /*Modal Content Load from page location*/
...
<div class="wrapper">
  ...
  <div class="content-wrapper" style="background-color: #ffffff; ">
    ...
    <form method="post" action="add-designation.php">
      <table class="table table-bordered">
        <tr>
        
          <th colspan="4" class="panel-heading" style="font-weight: bold; text-align: center;">Designation</th>
        </tr>   
        <tr>
        ...
        <?php
                                     ...
                                     $result = mysqli_query($conn,"SELECT MAX(cid)+1 FROM designation");
                                     $row = mysqli_fetch_row($result);
                                     $highest_id = $row[0];
                                    ?>
          <input type="hidden" name="txtcid" value="<?php echo   $highest_id; ?> " id="txtcid" class="form-control">
          <td><span style="color: #3973ac; font-weight: bold; ">Select Occupation</span></td>
          <td>
...
$sqli1 = "SELECT Name FROM occupation ";
$result = mysqli_query($conn,$sqli1);

echo "<select id='occupationid' name='occupationid' class='form-control'>";
echo "<option value='0'>Select occupation</option>";
...
echo "</select>";
        ...
        ?>
          ...
          <td><span style="color: #3973ac; font-weight: bold; ">Designation Name</span></td>
          <td><input type="text" name="txtdesignation" id="txtdesignation" class="form-control">(<font color="#DB2929" style="font-size: small">e.g.- Clerk</font>)<span id="spnError" style="color: Red; display: none">*Enter Characters Only.</span>
          </td>
        </tr>
...
<?php
              ...
              $get2 = mysqli_query($conn,"SELECT * FROM designation");
...
function delete_row(id)
{
      ...
      $.confirm({
        ...
        buttons: {
            confirm: function () {
               ...
               $.ajax({
               type: 'POST',
               url: 'add-designation-delete-code.php',
               data: ({ id: id }),
  ...
  $(document).ready(function() {  
    ...
    var designation1=$('#txtdesignation').val();
  ...
  else
  {
    $.ajax({
         type: 'POST',
         url: 'add-designation2.php',
         data: ({cid:cid, occupation:occupation ,designation1:designation1}),
         success: function(response) 
     {
             if(response == "1" || response == 1)
            {  
             ...
             {
                 errorAlert("Designation Not Added");
             }
         }
      });
  }
