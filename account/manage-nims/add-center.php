...
<head>
  ...
  <?php include '../../assets/dashboard-assets-css.php'; ?>
    ...
    <script type="text/javascript">
      ...
      {
        setTimeout(function(){
          $('#light_box').load('add-center-update-modal.php?id='+id); /*Modal Content Load from page location*/
  ...
  <div class="content-wrapper" style="background-color: #ffffff; ">
    ...
    <form method="post" action="add-board.php">
      <table class="table table-bordered">
        <tr>
          <th colspan="4" class="panel-heading" style="font-weight: bold; text-align: center;">Center</th>
        </tr>   
        <tr>
        ...
        <?php
                                     ...
                                     $result = mysqli_query($conn,"SELECT MAX(ID)+1 FROM centers");
...
echo "<select id='CityName' name='CityName' class='form-control'>";
...
echo "</select>";
        ...
        ?>
          ...
          <td><span style="color: #3973ac; font-weight: bold; ">Center Name</span></td>
          <td><input type="text" name="Name" id="Name" class="form-control" style="width: 200px;">(<font color="#DB2929" style="font-size: small">e.g.- Rajapeth</font>)<span id="spnError" style="color: Red; display: none">*Enter Characters Only.</span>
          </td>
        </tr>
...
<?php
              ...
              $get2 = mysqli_query($conn,"SELECT * FROM centers");
              while($row2 = mysqli_fetch_array($get2))
              {

                $id=$row2['ID'];
                $city_nm=$row2['CityName'];
                $center_nm=$row2['Name'];
...
function delete_row(id)
{
      ...
      $.confirm({
        ...
        buttons: {
            confirm: function () {
               ...
               $.ajax({
               type: 'POST',
               url: 'add-center-delete-code.php',
               data: ({ id: id }),
  ...
  else
  {
    $.ajax({
         type: 'POST',
         url: 'add-center2.php',
         data: ({ID:ID, CityName:CityName ,Name:Name }),
         success: function(response3) 
     {
             if(response3 == "1")
            {  
              successAlert("Center Added Successfully");   
                 setTimeout(function(){
                  window.location.href='add-center.php';
                  }, 1000); 
