...
<head>
  ...
  <script type="text/javascript">
      ...
      {
        setTimeout(function(){
          $('#light_box').load('add-course-type-update-modal.php?id='+id); /*Modal Content Load from page location*/
    ...
    <form method="post" action="add-class.php">
      <table class="table table-bordered">
        ...
        <?php
                                     ...
                                     $result = mysqli_query($conn,"SELECT MAX(cid)+1 FROM coursetype");
...
echo "</select>";
        ...
        ?>
          ...
          <td><input type="text" name="txtcourse" id="txtcourse" class="form-control">(<font color="#DB2929" style="font-size: small">e.g.- Backlog</font>)<span id="spnError" style="color: Red; display: none">*Enter Characters Only.</span>
          </td>
        </tr>
        <tr>
          <td><span style="color: #3973ac; font-weight: bold; ">Enter Course Duration</span></td>
          <td><input type="text" name="c_durt" id="c_durt" class="form-control"><span id="spnError" style="color: Red; display: none">*Enter Characters Only.</span>

          <td><span style="color: #3973ac; font-weight: bold; ">Enter class Duration</span></td>
          <td><input type="text" name="c_time" id="c_time" class="form-control"><span id="spnError" style="color: Red; display: none">*Enter Characters Only.</span></td>

        </tr>
        <tr>
          <td><span style="color: #3973ac; font-weight: bold; ">Enter Course Description </span></td>
          <td><input type="text" name="c_desc" id="c_desc" class="form-control"><span id="spnError" style="color: Red; display: none">*Enter Characters Only.</span></td>
...
function delete_row(id)
{
      ...
      $.confirm({
        ...
        buttons: {
            confirm: function () {
               ...
               $.ajax({
               type: 'POST',
               url: 'add-course-type-delete-code.php',
               data: ({ id: id }),
               ...
               if(response == "1" || response == 1 )
               {
                 ...
                 setTimeout(function(){
                   window.location.href='add-course-type.php';
                 }, 1000);
               }
               else
               {
  ...
  $(document).ready(function() {  
    ...
    var coursetype=$('#txtcourse').val();
    var c_durt=$('#c_durt').val();  
    ...
    {
       infoAlert("Please Enter Course Type");
    }
    else if(c_durt=='')
    {
    ...
    {
       infoAlert("Please Enter Class Time");
    }
    else if(c_desc=='')
    {
  ...
  else
  {
    $.ajax({
         type: 'POST',
         url: 'add-course-type2.php',
         data: ({cid:cid, course:course ,coursetype:coursetype ,c_durt:c_durt ,c_time:c_time ,c_desc:c_desc}),
         success: function(response3) 
     {
             if(response3 == "1")
            {  
              successAlert("Course Type Added Successfully"); 
                   setTimeout(function(){
                  window.location.href='add-course-type.php';
                  }, 1000); 
             ...
             {
                 errorAlert("Course Type Not Added");
             }
         }
      });
  }
